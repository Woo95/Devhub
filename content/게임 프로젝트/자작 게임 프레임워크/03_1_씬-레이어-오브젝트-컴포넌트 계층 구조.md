...
## **설계 및 구현 방식**
### `구조 설계 방향`
씬-레이어-오브젝트-컴포넌트 구조를 설계하면서, 게임 요소를 하나의 단일 계층으로 관리할지, **역할별로 분리된 계층 구조로 구성할지**를 먼저 고민했습니다. 단일 계층 구조는 구현이 단순하다는 장점이 있지만, 렌더링 순서와 업데이트 책임이 혼재되면서 규모가 커질수록 관리가 어려워지는 문제가 있었습니다. 

이를 해결하기 위해, **책임을 분리한 계층 구조를 도입**했으며, 씬은 전체 상태를 관리하고 레이어는 렌더링 순서와 그룹을 담당하며 오브젝트는 개별 엔티티를 표현하고 컴포넌트는 기능 단위로 분리하는 구조로 설계했습니다. 이를 통해 각 계층이 명확한 역할을 가지도록 하면서도, 기능 확장 시 특정 계층만 수정하면 되도록 하여 **유지보수성과 확장성을 확보**할 수 있었습니다.
