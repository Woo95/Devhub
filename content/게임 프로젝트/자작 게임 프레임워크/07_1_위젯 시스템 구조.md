...
## **설계 및 구현 방식**
### `구조 설계 방향`
위젯 시스템을 설계할 때, 씬 내부에서 위젯들을 직접 관리하면 구조가 복잡해질 수 있다는 점을 고민했습니다. 그래서 씬이 직접 위젯을 관리하는 대신, 위젯들의 집합체를 관리하는 `CSceneUI`를 두어 UI를 통합적으로 관리하도록 설계했습니다. 덕분에 UI가 없는 씬에서는 해당 객체를 아예 들고 있지 않아도 되어 **불필요한 관리 부담을 줄일 수 있었습니다**.

또한, 각각의 UI를 **계층 구조로 구성**해 렌더링 순서와 부모-자식 관계, 드래그/호버/클릭과 같은 상호작용을 자연스럽게 처리할 수 있도록 했습니다. 이를 위해 `CWidget`을 추상 부모 클래스로 두고, 이를 상속받아 사용자 정의 위젯을 쉽게 만들 수 있는 `CUserWidget` 구조를 설계했습니다. 덕분에 기본 UI 틀을 제공하면서도 확장성과 관리 편의성을 동시에 확보할 수 있었습니다.
