...
## **설계 및 구현 방식**
### `구조 설계 방향`
초기에는 입력 시스템을 빠르게 구현하기 위해 `CInput` 하나에서 키 상태 관리와 함수 실행까지 모두 처리하는 **중앙 집중형 구조로 설계**했습니다. 이 방식은 구현이 단순하고 직관적이라는 장점이 있었지만, 실제 기능을 확장하는 과정에서 입력 처리와 실행 로직이 강하게 결합되는 문제가 발생했습니다. 특히 키 단위로 함수가 직접 연결되는 구조이다 보니, 키 조합 입력 처리나 입력 재사용이 어려워지고, 기능이 늘어날수록 코드의 관리 부담이 커지는 한계를 확인할 수 있었습니다.

이를 해결하기 위해, 입력 시스템을 단순히 키에 함수를 연결하는 방식에서 벗어나, **입력 조건과 실행 로직을 분리하는 구조로 개선**했습니다. `CInputManager`는 매 프레임 키보드와 마우스 입력을 감지해 각 키와 버튼의 상태를 갱신하고, `CInputComponent`는 등록된 입력 조건과 비교해 조건이 만족되면 대응 함수들을 실행합니다. 이를 위해 여러 입력 조건과 실행 함수를 하나로 묶는 `FBinder` 구조를 도입하여, 키보드와 마우스 입력을 조합한 복합 조건이 만족될 때 등록된 함수가 실행되도록 구성했습니다.

또한 바인딩 데이터는 [[메모리 풀 라이브러리|메모리 풀]] 기반으로 관리하여 동적 할당 비용을 줄이고 런타임 성능을 안정화했으며, 입력 처리 로직과 게임 로직 간의 의존성을 줄이도록 구조를 분리해 결합도를 낮췄습니다. 그 결과 **입력 상태 관리, 조건 처리, 실행 로직이 분리된 구조**를 확보했고, **복합 입력 처리와 기능 확장 측면에서 유연한 입력 시스템을 구현**할 수 있었습니다.
