...
---
## **개요**
이 문서는 게임 내 **충돌 및 물리 시스템의 구조와 동작 원리**를 설명하며, **충돌체 관리, 충돌 판정, 물리적 반응 처리**를 어떻게 구현했는지를 정리합니다.

---
### 1. 공간 분할 기반 충돌 관리
게임 내 충돌 및 물리 시스템은 `CQuadTree` 기반 **공간 분할 알고리즘** 안에서 **충돌 채널 및 프로필**을 함께 활용하여 **불필요한 충돌 검사를 최소화**합니다. `CCollisionManager`가 **충돌 알고리즘을 제공**하며, 이를 통해 **선별된 충돌체**들의 실제 충돌 여부를 검사합니다.

![[Self-Made Game Framework (Media)#^b7e46a]]

---
### 2. 충돌 이벤트 처리 및 물리 연계 
**충돌체 쌍 단위로 충돌을 관리**하며, **충돌 이벤트**(ENTER/STAY/EXIT)를 전달합니다. 또한 **물리 처리가 필요한 경우**에는 `CPhysicsManager`와 `CRigidbody`가 겹침을 해소하고 물리적 반응을 처리하여 **안정적인 충돌 처리를 보장**합니다.
...
## **설계 및 구현 방식**
### `구조 설계 방향`
처음에는 유니폼 그리드 기반 충돌 시스템을 고려했으나, **충돌체가 수백 개 이상으로 증가하면 불필요한 충돌 검사와 메모리 낭비가 발생**하고, **그리드 크기와 오브젝트 분포에 따라 처리 효율이 크게 달라지는 문제**가 있었습니다. 이를 해결하기 위해 `CSceneCollision`과 `CQuadTree`를 도입하여 **충돌체를 씬 단위로 등록하고 영역별로 분할 관리**하며, 각 노드가 충돌체를 보관하고 필요 시 자식 노드로 분배하여 **검사 범위를 최소화**했습니다. 

또한, **충돌 채널 및 프로필을 활용**하여 오브젝트가 어떤 그룹과 충돌하여 반응을 할지를 정의함으로써 **불필요한 충돌 검사를 줄였습니다**. `CCollisionManager`가 충돌 계산을 제공하고, `CPhysicsManager`와 `CRigidbody`가 물리적 반응을 처리하도록 설계하여, 충돌 판정과 물리 반응을 **안정적이고 효율적으로 수행하며 시스템의 성능을 확보**할 수 있습니다.
