...
## **설계 및 구현 방식**
### `구조 설계 방향`
처음 설계할 때는 `CSpriteComponent`와 `CVFXComponent`가 **각 오브젝트의 애니메이션을 직접 관리**하도록 구현하려 했지만, 동일한 애니메이션을 여러 오브젝트가 사용할 경우 **동기화 문제와 메모리 중복 사용**이 발생하는 점이 고민이었습니다.

이를 해결하기 위해, `CSpriteComponent`와 `CVFXComponent`는 `CAnimation` 포인터 멤버 변수를 가지도록 구성하고, `CAnimationManager`에서 애니메이션을 가져와 사용하도록 만들었습니다. `CAnimation` 객체는 `std::shared_ptr`로 여러 오브젝트가 공유되더라도, 현재 프레임과 재생 상태 같은 진행 정보는 `CAnimation` 내부 멤버 변수로 인해서 **각 오브젝트는 독립적으로 표현되도록 구현**했습니다. 덕분에 **리소스 사용을 효율적으로 관리**할 수 있게 되었습니다.
