...
---
## **충돌 관련 클래스 목록 및 설명**
### `1. CCollisionManager 클래스`
![[Self-Made Game Framework (Code)#^f68f25]]
- **역할**:
	- **모든 충돌 관련 프로필을 저장하고 관리**합니다.
...
- **핵심 함수들**:
	...
	- [[Self-Made Game Framework (Code)#^c34b99|bool AABBCircleCollision(CBoxCollider* collider1, CCircleCollider* collider2)]]:
		- 박스와 원의 충돌을 **원의 중심과 가장 가까운 박스 테두리까지의 거리**로 판정하고, 겹치면 그 최근접점을 `mHitPoint`로 기록합니다.

---
### `2. CSceneCollision 클래스`
...
- **역할**:
	- **씬의 모든 충돌체를 관리**하고, 카메라 영역에 맞춰 `mRoot`를 **초기화 및 관리**합니다.
- **핵심 변수들**:
	- `mRoot`:
		- `CQTNode`의 **최상위 노드**입니다. 모든 충돌체는 이 노드를 기준으로 분배됩니다.
	- `mColliders`:
		- 씬에 존재하는 **모든 충돌체를 보관하는 컨테이너**입니다.
- **핵심 함수들**:
	- [[Self-Made Game Framework (Code)#^033b8a|CQuadTree(CCamera* camera)]]:
		- `MAX_SPLIT`을 기준으로 **전체 노드 수를 계산**한 뒤, 그 수만큼 `CQTNode` 전용 [[메모리 풀 라이브러리|메모리 풀]]을 생성하고, `mRoot`에 메모리를 할당하여 초기화합니다.
...
---
## **물리 관련 클래스 목록 및 설명**
### `1. CPhysicsManager 클래스`
![[Self-Made Game Framework (Code)#^6ed2ab]]
- **역할**:
	- 씬 내 충돌체 간 상호작용이 **물리적 처리 조건**을 만족하면, `CRigidbody` 타입에 따라 **최소 이동 벡터(MTV)를 계산**하여 **충돌체를 이동**시킵니다.
- **핵심 변수들**:
	- `CONST_MinMtvLSq`:
...
- **핵심 변수들**:
	...
	- `mMass`, `mGravityScale`, `mVelocity`, `mAcceleration`, `mAccumulatedForce`:
		- 질량과 중력 영향을 포함한 운동 상태와 누적 힘을 관리하며, **충돌 처리와 물리에 사용**됩니다.
- **핵심 함수**:
